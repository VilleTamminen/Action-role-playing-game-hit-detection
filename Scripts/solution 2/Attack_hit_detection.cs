...
{
    //This must be on child object, like emptry transform, so that it will only use that collider for weapon hit detection
    //This script is placed on weapons or anything that does damage. Just enable/disable collider(s) when necessary with Character_hit_detection.
